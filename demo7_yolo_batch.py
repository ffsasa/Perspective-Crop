...
# =========================
# Quad from mask by line fitting (AI-consistent, stable)
# =========================
def fit_line_ransac(points, iterations=200, threshold=2.0):
    best_line = None
    best_inliers = 0
...
def quad_from_mask_line_fitting(mask):
    ...
    v1 = eigvecs[:, order[0]]  # trục chính dài
    v2 = eigvecs[:, order[1]]  # trục còn lại

    # Chuẩn hóa
    v1 = v1 / (np.linalg.norm(v1) + 1e-6)
    v2 = v2 / (np.linalg.norm(v2) + 1e-6)

    # === Chiếu điểm lên 2 trục ===
    proj1 = centered @ v1
    proj2 = centered @ v2
    ...
    def fit_edge(points):
        if len(points) < 50:
            return None
        return fit_line_ransac(points)

    edge1_pts = pts[np.abs(proj1 - min1) < 3]
    edge2_pts = pts[np.abs(proj1 - max1) < 3]
    edge3_pts = pts[np.abs(proj2 - min2) < 3]
    edge4_pts = pts[np.abs(proj2 - max2) < 3]

    l1 = fit_edge(edge1_pts)
    l2 = fit_edge(edge2_pts)
    l3 = fit_edge(edge3_pts)
    l4 = fit_edge(edge4_pts)

    if any(l is None for l in [l1, l2, l3, l4]):
        return None

    # Giao điểm
    p1 = intersect_lines(l1, l3)
    p2 = intersect_lines(l1, l4)
    p3 = intersect_lines(l2, l4)
    p4 = intersect_lines(l2, l3)
...
# =========================
# Quad scoring & candidates (GIỮ theo bản YOLO của bạn)
# =========================
...
def detect_document(image_path, debug=False):
    ...
    # Stable quad from AI mask
    # Candidate quads (ưu tiên projective)
    cands = []

    # 1) approxPolyDP trên hull với nhiều eps để bắt được hình thang (perspective)
    cands += quad_candidates_from_mask(best_mask, eps_list=(0.005, 0.010, 0.015, 0.020, 0.030, 0.040, 0.060),
                                    min_area_ratio=0.15)

    # 2) "corner peaks" trên hull (hay bắt đúng 4 góc hình thang)
    q_peaks = quad_from_hull_corner_peaks(best_mask, simplify_eps=0.008)
    if q_peaks is not None:
        cands.append(q_peaks)

    # 3) minAreaRect (fallback affine)
    q_rect = minarea_box_from_mask(best_mask)
    if q_rect is not None:
        cands.append(q_rect)

    # 4) PCA box (fallback affine, ổn định)
    q_pca = pca_box_from_mask(best_mask)
    if q_pca is not None:
        cands.append(q_pca)

    best_q, best_s, best_reason = None, -1e18, "n/a"

    # Nới ngưỡng một chút để không “bỏ qua” các ca perspective mạnh
    for q in cands:
        s, reason = score_quad(
            best_mask, q,
            min_cover=0.85,   # nới
    ...
    if best_q is None:
        raise Exception("YOLO có mask nhưng không suy được quad hợp lệ (cands rỗng/score fail)")
